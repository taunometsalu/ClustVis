#Tauno muudetud:
#väiksed muudatused, et cluster_mat() aktsepteeriks ka "hclust" klassi objekti
#legend ja annotatsioonide legend lükata otse vasakule, kui pole reanimesid
#reanimede laius
#legendi pööramine
#legendi nimed üleval + joon
#eri värvi reanimed
#reanimede pealkiri
#joonise mõõtmete tagastamine (pole tegelikult vaja)


lo = function(rown, coln, nrow, ncol, cellheight = NA, cellwidth = NA, treeheight_col, treeheight_row, legend, annotation, annotation_colors, annotation_legend, annotation_titles, main, fontsize, fontsize_row, fontsize_col, ...){
	# Get height of colnames and length of rownames
	if(!is.null(coln[1])){
		#longest_coln = which.max(nchar(coln))
		longest_coln = which.max(strwidth(coln, units = 'in')) #Tauno lisa
	...
	if(!is.null(rown[1])){
		...
		#rown_width = unit(1.2, "grobwidth", textGrob(rown[longest_rown], gp = do.call(gpar, gp))) #Tauno muudetud
		rown_width = unit(1.0, "grobwidth", textGrob(rown[longest_rown], gp = do.call(gpar, gp))) + unit(3, "mm")
	}
	else{
	...
	if(is.na(main)){
		...
		main_height = unit(1.95, "mm") #Tauno muudetud, käsitsi proovitud konstant
	}
	...
	if(!is.na(annotation[[1]][1])){
		...
		#Tauno muudetud, et arvestada ka pealkirjade laiust (ja strwidth, mitte nchar):
		longest_vec = as.vector(rbind(as.matrix(annotation), colnames(annotation)))
		longest_ann = which.max(strwidth(longest_vec, units = 'in'))
		annot_legend_width = unit(1.2, "grobwidth", textGrob(longest_vec[longest_ann], gp = gpar(...))) + unit(12, "bigpts")
	...
	if(FALSE){
	#Tauno lisatud read: arvutame enne suuruse ja lisame ka dev.new(), et ekraanil avatuna tuleks õige suurus:
	widthWhole = unit.c(treeheight_row, matwidth, rown_width, legend_width, annot_legend_width)
	heightWhole = unit.c(main_height, treeheight_col, annot_height, matheight, coln_height)
	widthWholeInches = convertWidth(widthWhole, "inches", valueOnly = TRUE)
	heightWholeInches = convertHeight(heightWhole, "inches", valueOnly = TRUE)
	dev.new(width = widthWholeInches, height = heightWholeInches)
	}
	
	# Produce layout()
	vp = viewport(layout = grid.layout(nrow = 5, ncol = 5, 
		widths = unit.c(treeheight_row, matwidth, rown_width, legend_width, annot_legend_width), 
		heights = unit.c(main_height, treeheight_col, annot_height, matheight, coln_height)), gp = do.call(gpar, gp))
	pushViewport(vp) #Tauno muudetud - salvestame enne
	...
	return(list(mindim, vp)) #Tauno muudetud
}
...
draw_rownames = function(rown, rownames_colors, ...){
	n = length(rown)
	y = 1 - ((1:n)/n - 1/2/n)
	xcoord = unit(1, "mm") #Tauno lisa
	#xcoord = unit(0.04, "npc") #vaikimisi
	ncolors = length(rownames_colors) #Tauno lisa
	if(ncolors == 0){ #Tauno lisa
		grid.text(rown, x = xcoord, y = y, vjust = 0.5, hjust = 0, gp = gpar(...))
	} else { #kui on antud värvid, millega reanimedes värvida
		separ = " | "
		str = strsplit(rown, separ, fixed = TRUE)
		widths = sapply(str[[1]], nchar) #vaatame ainult esimest
		for(i in 1:length(widths)){
			#i = 2
			column = sapply(str, function(x) x[i])
			cum = cumsum(c(0, widths[-length(widths)]))
			nspaces = cum[i] + (i - 1) * nchar(separ) #mitu tühikut lisame ette, et ei kirjutaks üle
			columnSpaces = paste(paste(rep(" ", nspaces), collapse = ""), column, sep = "")
			uni = unique(column)
			if(length(uni) > ncolors | length(uni) == 1){
				grid.text(columnSpaces, x = xcoord, y = y, vjust = 0.5, hjust = 0, gp = gpar(...))
			} else {
				for(j in 1:length(uni)){ #määrame värvi gruppide kaupa
					#j = 1
					w = which(column == uni[j])
					grid.text(columnSpaces[w], x = xcoord, y = y[w], vjust = 0.5, hjust = 0, gp = gpar(col = rownames_colors[j], ...))
				}
			}
		}
		allSeparators = paste(sapply(widths, function(x) paste(rep(" ", x), collapse = "")), collapse = separ) #kõik eraldajad korraga
		grid.text(allSeparators, x = xcoord, y = y, vjust = 0.5, hjust = 0, gp = gpar(...))
	}
}

#Tauno lisa - pealkiri reanimede kohale
draw_rownames_title = function(title, ...){
	#un = 0.16 #siis võrdsed vahed
	#un = 0.35 #siis pealkiri veidi kõrgemal kui ülejäänud read
	un = 0.4
	#xcoord = unit(0.04, "npc") #vaikimisi
	xcoord = unit(1, "mm")
	grid.text(title, x = xcoord, y = unit(un, "npc"), vjust = 0.5, hjust = 0, gp = gpar(...))	
}
...
draw_annotations = function(converted_annotations, border_color){
	...
	for(i in 1:m){
		...
		grid.rect(x = x[i], unit(y[n:1], "bigpts"), width = 1/m, height = unit(8, "bigpts"), gp = gpar(fill = converted_annotations[i, ], col = border_color)) #Tauno muudatus, et legendid ja annotatsioonid oleks sama pidi
	}
}

#Tauno lisa - legendi pealkirjad heatmapi kohale ka
draw_annotation_labels = function(converted_annotations){
	n = ncol(converted_annotations)
	y = cumsum(rep(8, n)) - 4 + cumsum(rep(2, n))
	for(i in 1:n){
		grid.text(colnames(converted_annotations)[i], x = unit(0.04, "npc"), 
			hjust = 0, y = unit(y[n - i + 1], "bigpts"), gp = gpar(fontface = "bold")) 
	}
}
...
heatmap_motor = function(matrix, border_color, cellwidth, cellheight, tree_col, tree_row, treeheight_col, treeheight_row, filename, width, height, breaks, color, legend, annotation, annotation_colors, annotation_legend, annotation_titles, main, fontsize, fontsize_row, fontsize_col, fmat, fontsize_number, rownames_title, rownames_colors, ...){ #rownames_title ja rownames_colors Tauno lisatud
	grid.newpage()
	
	# Set layout
	lo = lo(coln = colnames(matrix), rown = rownames(matrix), nrow = nrow(matrix), ncol = ncol(matrix), cellwidth = cellwidth, cellheight = cellheight, treeheight_col = treeheight_col, treeheight_row = treeheight_row, legend = legend, annotation = annotation, annotation_colors = annotation_colors, annotation_legend = annotation_legend, annotation_titles = annotation_titles, main = main, fontsize = fontsize, fontsize_row = fontsize_row, fontsize_col = fontsize_col,  ...) #Tauno muudetud
	mindim = lo[[1]] #Tauno lisatud
	vp = lo[[2]] #Tauno lisatud
	...
	if(!is.na(filename)){
		...
		heatmap_motor(matrix, cellwidth = cellwidth, cellheight = cellheight, border_color = border_color, tree_col = tree_col, tree_row = tree_row, treeheight_col = treeheight_col, treeheight_row = treeheight_row, breaks = breaks, color = color, legend = legend, annotation = annotation, annotation_colors = annotation_colors, annotation_legend = annotation_legend, annotation_titles = annotation_titles, filename = NA, main = main, fontsize = fontsize, fontsize_row = fontsize_row, fontsize_col = fontsize_col, fmat = fmat, fontsize_number =  fontsize_number, rownames_title = rownames_title, rownames_colors = rownames_colors, ...) #rownames_title ja rownames_colors Tauno lisatud
		dev.off()
		upViewport()
		return()
	}
	...
	if(length(rownames(matrix)) != 0){
		...
		pars = list(rownames(matrix), fontsize = fontsize_row, rownames_colors = rownames_colors, ...) #rownames_colors Tauno lisatud
	...
	# Tauno lisatud
	# Draw rownames title
	if(!is.na(rownames_title)){
		pushViewport(vplayout(2, 3))
		pars = list(rownames_title, ...)
		do.call(draw_rownames_title, pars)
		upViewport()
	}
	...
	if(!is.na(annotation[[1]][1])){
		...
		upViewport()
    if(annotation_titles){ #Tauno lisatud - kas kirjutame soojuskaardi kohale ka legendi pealkirjad
      pushViewport(vplayout(3, 3)) 
      draw_annotation_labels(converted_annotation)
      upViewport()
    }
	...
	if(!is.na(annotation[[1]][1]) & annotation_legend){
		if(length(rownames(matrix)) != 0){
			...
			pushViewport(vplayout(4:5, 5)) #Tauno muudetud, et lükkaks otse vasakule
		}
	...
	if(!is.na(legend[1])){
		...
		if(length(rownames(matrix)) != 0){
			...
			pushViewport(vplayout(4:5, 4)) #Tauno muudetud, et lükkaks otse vasakule
		}
		draw_legend(color, breaks, legend, fontsize = fontsize, ...)
		upViewport()
	}
	
	invisible(vp) #Tauno lisatud
}
...
cluster_mat = function(mat, distance, method){
	#Tauno muudatused, et võtaks ka hclust() objekti sisse
	...
	if(!(distance[1] %in% c("correlation", "euclidean", "maximum", "manhattan", "canberra", "binary", "minkowski")) & !(class(distance) %in% c("dist", "hclust"))){
		print(!(distance[1] %in% c("correlation", "euclidean", "maximum", "manhattan", "canberra", "binary", "minkowski")) | !(class(distance) %in% c("dist", "hclust")))
		stop("distance has to be a structure as produced by dist or hclust or one measure form the list: 'correlation', 'euclidean', 'maximum', 'manhattan', 'canberra', 'binary', 'minkowski'")
	}
  
  if(class(distance) == "hclust"){
    hc = distance
  } else {
...
#'
#' @export
pheatmap = function(mat, color = colorRampPalette(rev(c("#D73027", "#FC8D59", "#FEE090", "#FFFFBF", "#E0F3F8", "#91BFDB", "#4575B4")))(100), kmeans_k = NA, breaks = NA, border_color = "grey60", cellwidth = NA, cellheight = NA, scale = "none", cluster_rows = TRUE, cluster_cols = TRUE, clustering_distance_rows = "euclidean", clustering_distance_cols = "euclidean", clustering_method = "complete",  treeheight_row = ifelse(cluster_rows, 50, 0), treeheight_col = ifelse(cluster_cols, 50, 0), legend = TRUE, legend_breaks = NA, legend_labels = NA, annotation = NA, annotation_colors = NA, annotation_legend = TRUE, annotation_titles = TRUE, drop_levels = TRUE, show_rownames = T, show_colnames = T, main = NA, fontsize = 10, fontsize_row = fontsize, fontsize_col = fontsize, display_numbers = F, number_format = "%.2f", fontsize_number = 0.8 * fontsize, filename = NA, width = NA, height = NA, rownames_title = NA, rownames_colors = NULL, ...){ #rownames_title, rownames_colors, annotation_titles Tauno lisatud
  ...
  if (legend & is.na(legend_breaks[1])) {
	...
	# Draw heatmap
	mot = heatmap_motor(mat, border_color = border_color, cellwidth = cellwidth, cellheight = cellheight, treeheight_col = treeheight_col, treeheight_row = treeheight_row, tree_col = tree_col, tree_row = tree_row, filename = filename, width = width, height = height, breaks = breaks, color = color, legend = legend, annotation = annotation, annotation_colors = annotation_colors, annotation_legend = annotation_legend, annotation_titles = annotation_titles, main = main, fontsize = fontsize, fontsize_row = fontsize_row, fontsize_col = fontsize_col, fmat = fmat, fontsize_number = fontsize_number, rownames_title = rownames_title, rownames_colors = rownames_colors,...) #rownames_title, rownames_colors ja annotation_titles Tauno lisatud, omistamine muutujale ka
	
	invisible(list(tree_row = tree_row, tree_col = tree_col, kmeans = km, viewport = mot)) #Tauno lisatud viewport
}
